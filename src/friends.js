    ...
    zone.addEventListener("drop", e => {
      if (currentDrag) {
        ...
        if (currentDrag.source !== zone) {
          ...
          if (
            currentDrag.node.parentNode.classList.contains(
              "friends-catalog__list--filter"
            )
          ) {
            const icon = currentDrag.node.querySelector(
              ".friend-card__control"
            );
...
function searchEngine(list, inputValue) {
  for (const card of list.children) {
    ...
    if (
      firstName.indexOf(inputValue) > -1 ||
      lastName.indexOf(inputValue) > -1
    ) {
